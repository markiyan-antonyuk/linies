...
open class DataReceiver(context: Context, private val onDataListener: (DataMap) -> Unit) : DataTransfer, DataApi.DataListener {
        ...
        dataEvents.apply {
            forEach {
                if (it.type == DataEvent.TYPE_CHANGED && it.dataItem.uri.path == DataTransfer.URI_PATH) {
                    ...
                    onDataListener(DataMapItem.fromDataItem(it.dataItem).dataMap)
                }
            }
            release()
        }
