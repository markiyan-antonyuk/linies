...
class DataSender(private val context: Context, override val protocol: Protocol) : DataTransfer {
    ...
    fun send(text: String? = null, configuration: Configuration? = null, filter: String = "", respondBack: Boolean = false) {
    ...
    private fun MessageClient.sendMessage(message: Message) = launch(CommonPool) {
        ...
        nodes.forEach { sendMessage(it, DataTransfer.REMOTE_PATH, message.toBytes()) }
    }
}
